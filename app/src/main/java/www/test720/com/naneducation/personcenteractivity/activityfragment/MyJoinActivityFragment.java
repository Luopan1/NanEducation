...
import www.test720.com.naneducation.bean.ActivityBean;
...
public class MyJoinActivityFragment extends BaseFragment {
    BaseRecyclerAdapter<ActivityBean.DataBean.ListBean> mAdapter;
    List<ActivityBean.DataBean.ListBean> mLists = new ArrayList<>();
    ...
    protected void getData() {
        ...
        mSubscription = mHttpUtils.getData(UrlFactory.userJoinActlist, params, 1).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<String>() {
            @Override
            public void onStart() {
            }
            ...
            public void onNext(String s) {
                ...
                Gson gson = new Gson();
                ActivityBean bean = gson.fromJson(s, ActivityBean.class);
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter<ActivityBean.DataBean.ListBean>(getActivity(), mLists, R.layout.item_group_activty_item) {
                @Override
                public void convert(BaseRecyclerHolder holder, ActivityBean.DataBean.ListBean item, int position, boolean isScrolling) {
