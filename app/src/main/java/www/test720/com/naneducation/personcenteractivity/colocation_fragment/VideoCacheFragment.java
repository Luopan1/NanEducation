...
import www.test720.com.naneducation.bean.VideoBean;
...
public class VideoCacheFragment extends BaseFragment {
    ...
    private List<VideoBean.DataBean.ListBean> mLists = new ArrayList<>();
    BaseRecyclerAdapter<VideoBean.DataBean.ListBean> mAdapter;
    int mCurrentPage = 1;
    int mTotalPage = 0;
    ...
    protected void getData() {
        ...
        mSubscription = mHttpUtils.getData(UrlFactory.userLoveList, params, 1).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<String>() {
            ...
            public void onNext(String s) {
                ...
                VideoBean colocation = gson.fromJson(s, VideoBean.class);
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter<VideoBean.DataBean.ListBean>(getActivity(), mLists, R.layout.item_my_colocation_video_item) {
                @Override
                public void convert(BaseRecyclerHolder holder, VideoBean.DataBean.ListBean item, int position, boolean isScrolling) {
