...
import www.test720.com.naneducation.bean.SchoolColocatonBean;
...
public class SchoolFragment extends BaseFragment {
    ...
    private List<SchoolColocatonBean.DataBean.ListBean> mLists = new ArrayList<>();
    BaseRecyclerAdapter<SchoolColocatonBean.DataBean.ListBean> mAdapter;
    int mCurrentPage = 1;
    int mTotalPage = 0;
    ...
    protected void getData() {
        ...
        mSubscription = mHttpUtils.getData(UrlFactory.userLoveList, params, 1).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<String>() {
            ...
            public void onNext(String s) {
                ...
                SchoolColocatonBean bean = gson.fromJson(s, SchoolColocatonBean.class);
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter<SchoolColocatonBean.DataBean.ListBean>(getActivity(), mLists, R.layout.item_my_colocation_school_item) {
                @Override
                public void convert(BaseRecyclerHolder holder, SchoolColocatonBean.DataBean.ListBean item, int position, boolean isScrolling) {
                    holder.setImageByUrl(R.id.couseImage, UrlFactory.baseImageUrl + item.getS_logo());
                    holder.setText(R.id.address, item.getS_address());
                    holder.setText(R.id.courseTitle, item.getS_name());
                }
            };
            ...
            mAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
                ...
                public void onItemClick(RecyclerView parent, View view, int position) {
                    ...
                    bundle.putString("title", mLists.get(position).getS_name());
