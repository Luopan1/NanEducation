...
import www.test720.com.naneducation.bean.CiurseBean;
...
public class CourseFragment extends BaseFragment {
    ...
    private List<CiurseBean.DataBean.ListBean> mLists = new ArrayList<>();
    BaseRecyclerAdapter<CiurseBean.DataBean.ListBean> mAdapter;
    int mCurrentPage = 1;
    int mTotalPage = 0;
    ...
    protected void getData() {
        ...
        mSubscription = mHttpUtils.getData(UrlFactory.userLoveList, params, 1).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<String>() {
            ...
            public void onNext(String s) {
                ...
                CiurseBean bean = gson.fromJson(s, CiurseBean.class);
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter<CiurseBean.DataBean.ListBean>(getActivity(), mLists, R.layout.item_my_colocation_course_item) {
                @Override
                public void convert(BaseRecyclerHolder holder, CiurseBean.DataBean.ListBean item, int position, boolean isScrolling) {
